...
# FIXME: add import from rkllm module here once it's ready
from api.rkllm import RKLLMLoaderClass
from api.rkllm import available_models
from api.rkllm import model_configs


# TODO:
# - move to loguru
# - correctly pass all supported sampling parameters
# - raise errors for everything unsupported
# - validate that the history can be retained and managed correctly
...
class Message(BaseModel):
  role: Literal["system", "user", "assistant"]  # This restricts the role to these three values
  content: str
...
def get_RKLLM_output(
  ...
  try:
    ...
    for item in output_gen:
      last_content.append(item)  # In case it's just a string, we use it as the final output
  ...
  try:
    ...
    for item in output_gen:
      ...
      if item:  # not the last token
        ...
        # Create the StreamChoice object with finish reason
        stream_choice = StreamChatChoice(delta=steam_message, finish_reason="stop", index=0)
...
async def create_completion(request: CompletionRequest):
  try:
    ...
    frequency_penalty = (
      request.frequency_penalty if request.frequency_penalty is not None else base_config["frequency_penalty"]
    )
    presence_penalty = (
      request.presence_penalty if request.presence_penalty is not None else base_config.get("presence_penalty", 0.0)
    )
